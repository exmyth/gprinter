...
public class GpTools {
	public static Vector<Byte> convertHexadecimalToBinary(Vector<Byte> HexadecimalData) {
		...
		Vector<Byte> binaryData = new Vector(hexLength / 2, 1024);

		for (index2 = 1; index2 < hexLength; index2 += 2) {
			index1 = index2 - 1;
			binaryValue = (byte) (xtod(((Byte) HexadecimalData.elementAt(index1)).byteValue()) * 16
					+ xtod(((Byte) HexadecimalData.elementAt(index2)).byteValue()));
	...
	public static Vector<Byte> convertEscposToBinary(String escpos) {
		...
		Vector<Byte> binaryData = new Vector(100, 50);
		try {
