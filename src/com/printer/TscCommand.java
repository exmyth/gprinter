...
public class TscCommand {
	...
	/*
	 * public void addBitmap(int x, int y, BITMAP_MODE mode, Bitmap b) { Matrix
	 * matrix = new Matrix(); matrix.postScale(2.0F, 2.0F); Bitmap bmp =
	 * Bitmap.createBitmap(b, 0, 0, b.getWidth(), b.getHeight(), matrix, true);
	 * 
	 * Bitmap NmBinaryBitmap = GpUtils.toBinaryImage(bmp, bmp.getWidth(), 16);
	 * Log.d("TSCCommand", "addBitmapt"); if (NmBinaryBitmap != null) { int i =
	 * 8 * (bmp.getWidth() / 8); int j = i * NmBinaryBitmap.getHeight() /
	 * NmBinaryBitmap.getWidth(); byte[] codecontent = null; codecontent =
	 * GpUtils.pixToTSCCmd(x, y, mode.getValue(), GpUtils.bitmapToBWPix(GpUtils
	 ...
	 * k++) { this.Command.add(Byte.valueOf((byte) 10)); } Log.d("TSCCommand",
