...
public class GpUtils {
	...
	/*
	 * public static Bitmap resizeImage(Bitmap bitmap, int w, int h) { Bitmap
	 * BitmapOrg = bitmap;
	 * 
	 * int width = BitmapOrg.getWidth(); int height = BitmapOrg.getHeight(); int
	 * newWidth = w; int newHeight = h;
	 * 
	 * float scaleWidth = newWidth / width; float scaleHeight = newHeight /
	 * height;
	 * 
	 ...
	 * 
	 * public static Bitmap toGrayscale(Bitmap bmpOriginal) { int height =
	 * bmpOriginal.getHeight(); int width = bmpOriginal.getWidth(); Bitmap
	 * bmpGrayscale = Bitmap.createBitmap(width, height, Bitmap.Config.RGB_565);
	 * Canvas c = new Canvas(bmpGrayscale); Paint paint = new Paint();
	 * ColorMatrix cm = new ColorMatrix(); cm.setSaturation(0.0F);
	 * ColorMatrixColorFilter f = new ColorMatrixColorFilter(cm);
	 * paint.setColorFilter(f); c.drawBitmap(bmpOriginal, 0.0F, 0.0F, paint);
	 * return bmpGrayscale; }
	 */
	...
	/*
	 * public static byte[] bitmapToBWPix(Bitmap mBitmap) { int[] pixels = new
	 * int[mBitmap.getWidth() * mBitmap.getHeight()]; byte[] data = new
	 * byte[mBitmap.getWidth() * mBitmap.getHeight()]; Bitmap grayBitmap =
	 * toGrayscale(mBitmap); grayBitmap.getPixels(pixels, 0, mBitmap.getWidth(),
	 ...
	 * int[] pixels = new int[0]; switch (algorithm) { case 8: Bitmap grayBitmap
	 * = toGrayscale(mBitmap); pixels = new int[grayBitmap.getWidth() *
	 * grayBitmap.getHeight()]; grayBitmap.getPixels(pixels, 0,
	 * grayBitmap.getWidth(), 0, 0, grayBitmap.getWidth(),
	 * grayBitmap.getHeight()); format_K_dither8x8_int(pixels,
	 * grayBitmap.getWidth(), grayBitmap.getHeight(), pixels); break; case 2:
	 * break; case 16: default: Bitmap grayBitmap1 = toGrayscale(mBitmap);
	 ...
	 * return pixels; }
	 * 
	 * public static Bitmap toBinaryImage(Bitmap mBitmap, int nWidth, int
	 * algorithm) { int width = (nWidth + 7) / 8 * 8; int height =
	 * mBitmap.getHeight() * width / mBitmap.getWidth(); Bitmap rszBitmap =
	 * resizeImage(mBitmap, width, height);
	 * 
	...
	public static byte[] pixToTSCCmd(int x, int y, int mode, byte[] paramArrayOfByte, int paramInt1, int paramInt2) {
		...
		for (int k = 0; k < arrayOfByte.length; k++) {
			...
			byte temp = (byte) (p0[paramArrayOfByte[j]] + p1[paramArrayOfByte[(j + 1)]] + p2[paramArrayOfByte[(j + 2)]]
					+ p3[paramArrayOfByte[(j + 3)]] + p4[paramArrayOfByte[(j + 4)]] + p5[paramArrayOfByte[(j + 5)]]
	...
	public static byte[] pixToESCCmd(byte[] paramArrayOfByte, int paramInt1, int paramInt2) {
		...
		for (int k = 8;; k++) {
			if (k >= arrayOfByte.length)
				return arrayOfByte;
			arrayOfByte[k] = ((byte) (p0[paramArrayOfByte[j]] + p1[paramArrayOfByte[(j + 1)]]
					+ p2[paramArrayOfByte[(j + 2)]] + p3[paramArrayOfByte[(j + 3)]] + p4[paramArrayOfByte[(j + 4)]]
					+ p5[paramArrayOfByte[(j + 5)]] + p6[paramArrayOfByte[(j + 6)]] + paramArrayOfByte[(j + 7)]));
			j += 8;
		}
	}
}
